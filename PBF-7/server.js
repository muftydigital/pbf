...
// Gunakan port 3000 sesuai pengaturan pada file .replit.
const port = 3000;
...
const server = createServer(async (request, response) => {
  try {
    ...
    // Gunakan index.html saat pengguna membuka halaman utama.
    ...
    try {
      ...
      // Kalau file tidak ada, arahkan kembali ke halaman utama React.
      filePath = join(distFolder, 'index.html');
    }

    // Baca isi file yang akan dikirim kepada browser.
    const fileContent = await readFile(filePath);
    // Pilih tipe konten berdasarkan ekstensi file.
